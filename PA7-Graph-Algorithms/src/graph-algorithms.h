...
std::list<value_type<T>> topologicalSort(const WeightedGraph<T>& graph)
{
    ...
    for(auto vertex : graph) {
        if(indegrees[vertex.first] == 0) q.push(vertex.first);
    }
    ...
    while(!q.empty()) {
        ...
        for(auto edge : graph.at(u)) {
            ...
            if(indegrees[edge.first] == 0) q.push(edge.first);
        }
    }

    if(l.size() != graph.size()) l.clear();
...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T>& graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    while(!q.empty()) {
        auto u = q.top();
        if(distances.at(u) == infinity<T>()) break;
        q.pop();
    ...
    while(predecessors.at(node).has_value()) {
        l.push_front(node);
        node = predecessors.at(node).value();
    }

    if(!l.empty() || initial_node == destination_node) l.push_front(initial_node);
    return l;
}
