...
inline bool operator==(const Queue<T, Container>& lhs, const Queue<T, Container>& rhs) {
    if(lhs.size() != rhs.size() || lhs.empty() || rhs.empty()) return false;
    auto leftIter = lhs.c.begin();
    auto rightIter = rhs.c.begin();
    while(leftIter != lhs.c.end() && rightIter != rhs.c.end()) {
        if(*leftIter != *rightIter) return false;
        ++leftIter;
        ++rightIter;
    }
    return true;
